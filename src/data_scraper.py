...
class DiksiyonaryoPHScraper:
    ...
    def print_words(self, words: Dict) -> None:
        ...
        :return: Nothing
        """
        with open("../data/raw/scraped_data.json.json", "w+", encoding="utf-8") as f:
    ...
    def scrape(self) -> Dict:
        ...
        for letter in self.letters:
            ...
            current_url = "{url}/list/{letter}".format(url=self.url, letter=letter)
    ...
    def _get_next_page(self, current_page: HTML):
        ...
        next_page_html = self._get_url_content(
            "{url}{next_page}".format(
    ...
    def _get_page_content(self, htmls):
        ...
        for html in htmls:
            ...
            for word_div in words_div:
                ...
                words[word] = {
                    "pronunciation": _get_text_if_exists(
                        word_div.find(".pronunciation", first=True)
                    ),
                    "part_of_speech": _get_text_if_exists(
                        word_div.find(".pos", first=True)
                    ),
                    "etymology": _get_text_if_exists(
                        word_div.find(".etymology", first=True)
                    ),
                    "derivative": _get_text_if_exists(
                        word_div.find(".derivative", first=True)
                    ),
